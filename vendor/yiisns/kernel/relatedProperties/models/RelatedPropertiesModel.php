...
/**
 *
 * @property RelatedPropertyModel[] $properties Class RelatedPropertiesModel
 * @package yiisns\kernel\relatedProperties\models
 */
class RelatedPropertiesModel extends DynamicModel
{
     ...
     *            list of attribute names that need to be saved. Defaults to null,
    ...
    protected function _saveRelatedPropertyValue($property)
    {
        ...
        if ($property->handler->isMultiple) {
            $propertyValues = $element->getRelatedElementProperties()
                ->where([
            ...
            if ($productPropertyValue = $element->getRelatedElementProperties()
                ->where([
                'property_id' => $property->id
            ])
                ->one()) {
    ...
    public function setAttribute($name, $value)
    {
        if ($this->hasAttribute($name)) {
            ...
            throw new InvalidParamException(get_class($this) . ' ' . \Yii::t('yiisns/kernel', 'has no attribute named "{name}".', [
                'name' => $name
            ]));
        }
    }
