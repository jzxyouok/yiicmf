 ...
 *
 * @package yiisns\kernel\dbDumper
 */
class DbDumperComponent extends Component
{
    ...
    public function init()
    {
        ...
        if (! $this->connection || ! $this->connection instanceof Connection) {
            throw new \InvalidArgumentException(\Yii::t('yiisns/dbDumper', 'Incorrect connection to the database'));
        }
    }
    ...
    public function dump()
    {
        ...
        if (! is_dir($this->backupDirPath)) {
            throw new \InvalidArgumentException(\Yii::t('yiisns/dbDumper', 'Folder to store the backup file is not found and could not be created') . ": " . $this->backupDirPath);
        }
    ...
    public function restore($fileDumpSql = null)
    {
        ...
        if (! $fileDumpSql) {
            if (! is_dir($this->backupDirPath)) {
                throw new \InvalidArgumentException(\Yii::t('yiisns/dbDumper', 'Do not locate the folder with the backup database') . ': ' . $this->backupDirPath);
            }
            
            if (! $files = FileHelper::findFiles($this->backupDirPath)) {
                throw new \InvalidArgumentException(\Yii::t('yiisns/dbDumper', 'Backup files found in a dir') . ': ' . $this->backupDirPath);
            }
        ...
        if (! file_exists($filePath) || ! is_readable($filePath)) {
            throw new \InvalidArgumentException(\Yii::t('yiisns/dbDumper', 'Dump file is not found'));
        }
        
        $sql = file_get_contents($filePath);
        
        if (! $sql) {
            throw new \InvalidArgumentException(\Yii::t('yiisns/dbDumper', 'Sql query is invalid'));
        }
