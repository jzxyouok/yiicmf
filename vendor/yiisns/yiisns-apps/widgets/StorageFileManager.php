...
class StorageFileManager extends Widget
{
    ...
    public function init()
    {
        ...
        {
            $this->backendSimpleUploadUrl = Url::to(['/admin/admin-storage-files/upload']);
        }
        if (!$this->backendRemoteUploadUrl)
        {
            $this->backendRemoteUploadUrl = Url::to(['/admin/admin-storage-files/remote-upload']);
        }
        ...
        \yiisns\admin\assets\ActionFilesAsset::register($this->getView());
...
JS
    ...
    public function run()
    {
        $str_upload = \Yii::t('yiisns/kernel','Upload');
        $str_togg_drop = \Yii::t('yiisns/kernel','Toggle Dropdow');
        $str_upl_from_comp = \Yii::t('yiisns/kernel','Upload from your computer');
        $str_remote_upl = \Yii::t('yiisns/kernel','Upload by link {http}',['http' => 'http://']);
        $str_up_f = \Yii::t('yiisns/kernel','The upload file');
        $str_up_fs = \Yii::t('yiisns/kernel','The upload files');
    return <<<HTML
