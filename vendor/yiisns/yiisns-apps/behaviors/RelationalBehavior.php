 ...
 * @date 10.10.2016
 */
namespace yiisns\apps\behaviors;
...
class RelationalBehavior extends Behavior
{
    ...
    public function attach($owner)
    {
        if (! ($owner instanceof ActiveRecord)) {
            throw new ErrorException(\Yii::t('yiisns/kernel', 'Owner must be instance of {yii}', [
                'yii' => 'yii\db\ActiveRecord'
            ]));
        }
        if (count($owner->getTableSchema()->primaryKey) > 1) {
            throw new ErrorException(\Yii::t('yiisns/kernel', 'RelationalBehavior doesn\'t support composite primary keys'));
        }
        parent::attach($owner);
    }
        ...
        foreach ($relatedRecords as $relationName => $relationRecords) {
            ...
            if (! empty($activeQuery->via)) { // works only for many-to-many relation
                ...
                if ($activeQuery->via instanceof ActiveQuery) {
                    ...
                    throw new ErrorException(\Yii::t('yiisns/kernel', 'Unknown via type'));
                }
                ...
                if ($passedRecords != $savedRecords) {
                    throw new ErrorException(\Yii::t('yiisns/kernel', 'All relation records must be saved'));
                }
                ...
                $model->getDb()->transaction(function () use($junctionTable, $primaryModelColumn, $relatedModelColumn, $junctionRows, $model) {
                    ...
                    $db->createCommand()
                        ...
                        $primaryModelColumn => $model->primaryKey
                    ])
                        ->execute();
                    if (! empty($junctionRows)) {
                        $db->createCommand()
                            ->batchInsert($junctionTable, [
                            $primaryModelColumn,
                            $relatedModelColumn
                        ], $junctionRows)
                            ->execute();
