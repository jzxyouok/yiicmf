 ...
 * 此页面有问题，当点击这个超链接显示此页面会出现左侧导航菜单连接失效，JS出现冲突
 */
$allCkecks = [];
...
?>
<h2><?= \Yii::t('yiisns/kernel', 'Full system testing')?></h2>
<p>
    <?=\Yii::t('yiisns/kernel', 'Full system scan helps to find the causes of the problems in the work site, and avoid errors in the future. Help for each test will help eliminate the cause of the error.')?>
</p>
<div class="sx-box sx-p-10 sx-bg-primary sx-checker">
	<p>
		<a href="#" class="btn btn-primary sx-controll-start"><i
			class="glyphicon glyphicon-play"></i> <?= \Yii::t('yiisns/kernel', 'Start testing') ?></a>
		<a href="#" class="btn btn-default sx-controll-stop"><i
			class="glyphicon glyphicon-stop"></i> <?= \Yii::t('yiisns/kernel', 'Stop') ?></a>
	</p>
	<p>
	<div class="sx-progress-tasks" id="sx-progress-tasks" style="display: none;">
		<span style="vertical-align: middle;"><h3><?= \Yii::t('yiisns/kernel', '"Testing the system (Progress"') ?> <span
					class="sx-executing-ptc"></span>%)
			</h3></span> <span style="vertical-align: middle;"><?= \Yii::t('yiisns/kernel','Test') ?>: <span
			class="sx-executing-task-name"></span></span>
		<div>
    ...
    <? foreach(\Yii::$app->getModules() as $module) : ?>
        <? if (method_exists($module, 'loadChecksComponents') ) : ?>
            <? if ($checks = $module->loadChecksComponents() ) : ?>
                ...
                <hr />
	<h3>Module <?= $module->getName(); ?> (Checks: <?= count($checks); ?>)</h3>
                <?=\yiisns\admin\widgets\GridView::widget(['dataProvider' => new \yii\data\ArrayDataProvider(['allModels' => $checks]),'layout' => "{items}",'columns' => [['attribute' => "name",'label' => 'Тест'],['class' => \yii\grid\DataColumn::className(),'value' => function (\yiisns\apps\base\CheckComponent $model) {$resultId = str_replace("\\", '-', $model->className());$result = \yii\helpers\Html::tag("div", "-", ['class' => 'sx-result-container','id' => $resultId]);return $result;},'format' => 'raw'],['class' => \yii\grid\DataColumn::className(),'value' => function (\yiisns\apps\base\CheckComponent $model) {$optionsJson = \yii\helpers\Json::encode(['title' => $model->name,'content' => $model->description]);return \yii\helpers\Html::a("<i class='glyphicon glyphicon-question-sign'></i>", "#", ['class' => 'btn btn-default','onclick' => "sx.dialog({$optionsJson}); return false;"]);},'format' => 'raw']]])?>
            <? endif; ?>
        <? endif; ?>
...
<?
\yiisns\apps\assets\JsTaskManagerAsset::register($this);
...
$optionsJs = [
    'checks' => $checksAll,
    'backend' => \yiisns\apps\helpers\UrlHelper::construct('admin/checker/check-test')->enableAdmin()->toString()
];
        ...
        _initQuery: function()
        {
            ...
            this.get("ajaxQuery").onSuccess(function(e, data)
            {
                if (data.response.success)
                {
                    ...
                    //Test has not completed yet another request must be made.
