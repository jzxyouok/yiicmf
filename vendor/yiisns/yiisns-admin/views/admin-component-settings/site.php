 ...
 * @var $site \yiisns\kernel\models\Site
 */
/* @var $this yii\web\View */
?>
<?= $this->render('_header', ['component' => $component]); ?>
    <h2><?=\Yii::t('yiisns/kernel', 'Settings for the site')?>: <?= $site->name; ?> (<?= $site->code; ?>)</h2>
    <div class="sx-box sx-mb-10 sx-p-10">
        <? if ($settings = \yiisns\kernel\models\ComponentSettings::fetchByComponentSiteCode($component, $site->code)) : ?>
            <button type="submit" class="btn btn-danger btn-xs" onclick="sx.ComponentSettings.Remove.removeBySite('<?= $site->code; ?>'); return false;">
                <i class="glyphicon glyphicon-remove"></i> <?=\Yii::t('yiisns/kernel', 'reset settings for this site')?>
            </button>
