 ...
 * Module是模块和应用程序类的基类
 *
 * 模块表示一个本身包含MVC元素（如模型，视图，控制器等）的子应用程序。
 *
 * 模块可以由 [[modules|sub-modules]] 组成。
 *
 * [[components|Components]] 可以在模块中注册，以便在模块中全局访问它们。
 *
 * 有关模块的更多细节和使用信息, 请参阅《[关于模块的指南文章](guide:structure-modules)》。
 * 
 * @property array $aliases 要定义的路径别名列表。数组 `键` 是别名（必须以 `@` 开头），数组 `值` 是路径或者别名。有关示例, 请参见 [[setAliases()]]。此属性为只写。
 * @property string $basePath 模块的根目录
 * @property string $controllerPath 包含控制器类的目录。此属性为只读
 * @property string $layoutPath 布局文件的根目录。默认为 "[[viewpath]]/layouts"。
