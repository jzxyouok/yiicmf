 ...
 * @since 1.0.0
 */
namespace yii\widget\simpleajaxuploader\backend;
...
class FileUpload
{
    ...
    public $uploadDir;
 // File upload directory (include trailing slash)
    public $allowedExtensions;
 // Array of permitted file extensions
    public $sizeLimit = 10485760000;
 // Max file upload size in bytes (default 10GB)
    public $newFileName;
 // Optionally save uploaded files with a new name by setting this
 ...
 // File extension of uploaded file
    private $savedFile;
 // Path to newly uploaded file (after upload completed)
    private $errorMsg;
 // Error message if handleUpload() returns false (use getErrorMsg() to retrieve)
    private $handler;
