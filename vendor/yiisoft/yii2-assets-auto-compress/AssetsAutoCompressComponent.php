 ...
 * @date 05.08.2016
 */
namespace yii\assetsAuto;

use yii\assetsAuto\components\HtmlCompressor;
...
class AssetsAutoCompressComponent extends Component implements BootstrapInterface
{
    ...
    protected function _processing(View $view)
    {
        ...
        // Compiling the .js code that appears on the page
        ...
        // Compiling CSS files that appears on the page
        ...
        // To move the CSS files down to the page where the .js view files are: POS_END
...
JS
                ...
                if (ArrayHelper::getValue($view->jsFiles, View::POS_END)) {
                    $view->jsFiles[View::POS_END] = ArrayHelper::merge($view->jsFiles[View::POS_END], [
                        $script
                    ]);
                } else {
    ...
    protected function _processingJs($parts)
    {
        ...
        if ($parts) {
            foreach ($parts as $key => $value) {
                $result[$key] = \JShrink\Minifier::minify($value, [
                    'flaggedComments' => $this->jsCompressFlaggedComments
                ]);
            }
        }
    ...
    protected function _processingCssFiles($files = [])
    {
        ...
        if (file_exists($rootUrl)) {
            ...
            foreach ($files as $fileCode => $fileTag) {
                if (Url::isRelative($fileCode)) {} else {
